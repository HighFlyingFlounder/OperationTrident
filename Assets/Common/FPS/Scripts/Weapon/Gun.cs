...
namespace OperationTrident.Weapon {
    ...
    public class Gun : MonoBehaviour, IShoot {
        ...
        [Tooltip("开镜时枪的位置")][SerializeField]
        private Transform MirrorTarget;
        [Tooltip("场景主摄像机")][SerializeField] private Camera m_MainCamera;
        [Tooltip("输入管理器")][SerializeField] private InputManager m_InputManager;
        ...
        public Ray GetShootRay() {
            ...
            Vector3 point;
            if (m_IsUsingMirror) {
                point = new Vector3(MirrorCamera.pixelWidth / 2, MirrorCamera.pixelHeight / 2, 0);
                ray = MirrorCamera.ScreenPointToRay(point);
            } else {
                point = new Vector3(m_MainCamera.pixelWidth / 2, m_MainCamera.pixelHeight / 2, 0);
                ray = m_MainCamera.ScreenPointToRay(point);
            }
